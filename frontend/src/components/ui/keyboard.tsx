...
interface KeyboardContextType {
  ...
  simulateKeyStroke: (keyCode: string, durationMs?: number) => void;
}
...
const KeyboardProvider = ({
  ...
  const simulateKeyStroke = useCallback(
    (keyCode: string, durationMs: number = 130) => {
...
const KeyboardContent = ({
  ...
  showTypedDisplay = true,
}: {
  containerRef: RefObject<HTMLDivElement | null>;
  className?: string;
  showPreview?: boolean;
  autoTypeText?: string;
  showTypedDisplay?: boolean;
}) => {
  const { typedText, setTypedText, simulateKeyStroke } = useKeyboardSound();

  useEffect(() => {
    if (!autoTypeText) return;

    let isCancelled = false;
    const timers: ReturnType<typeof setTimeout>[] = [];

    const runAutoType = async () => {
      // Delay before typing starts so user sees the page open
      await new Promise<void>((resolve) => {
        timers.push(setTimeout(resolve, 800));
      });
      if (isCancelled) return;

      setTypedText("");

      for (let i = 0; i < autoTypeText.length; i++) {
        if (isCancelled) return;
        const char = autoTypeText[i];
        const keyCode = CHAR_TO_KEY[char.toLowerCase()];

        if (keyCode) {
          simulateKeyStroke(keyCode, 130);
        }

        setTypedText(autoTypeText.slice(0, i + 1));

        // Natural typing cadence
        const delay =
          char === " " ? 340 : char === "." ? 400 : 170 + Math.floor(Math.random() * 80);
  ...
  return (
    <KeyboardProvider enableSound={enableSound} containerRef={containerRef}>
      <KeyboardContent
        ...
        showTypedDisplay={showTypedDisplay}
      />
    </KeyboardProvider>
  );
};
...
export const Keypad = () => {
  return (
    <div className="h-full w-fit rounded-xl bg-neutral-200 p-1 border border-neutral-300">
      {/* Function Row */}
      <Row>
        <Key
          keyCode="Escape"
...
const Key = ({
  ...
  const { playSoundDown, playSoundUp, pressedKeys, setPressed, setReleased, setTypedText } =
    useKeyboardSound();
  ...
  const handleMouseDown = () => {
    if (keyCode) {
      ...
      if (keyCode.startsWith("Key")) {
        setTypedText((prev) => prev + keyCode.slice(3).toLowerCase());
      } else if (keyCode.startsWith("Digit")) {
        setTypedText((prev) => prev + keyCode.slice(5));
  ...
  return (
    <div className={cn("rounded-[4px] p-[0.5px]", containerClassName)}>
      <button
        ...
        className={cn(
          "flex h-6 w-6 cursor-pointer items-center justify-center rounded-[3.5px] bg-gray-100 border border-neutral-300 transition-transform duration-75 active:scale-[0.98]",
          isPressed &&
            "scale-[0.98] bg-gray-200/90 border-neutral-400",
          className,
        )}
      >
        <div
